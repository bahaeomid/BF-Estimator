...
const BodyFatEstimator = () => {
  ...
  const getGalleryRanges = () => {
    ...
    return Object.entries(ranges)
      .map(([range, entries]) => ({
        ...
        avgBF: (entries.reduce((sum, e) => sum + e.bodyFat, 0) / entries.length).toFixed(1)
      }))
  ...
  const capturePhoto = async () => {
    ...
    reader.onloadend = () => {
      setPhotos({ ...photos, [currentPhotoType]: reader.result });
      stopCamera();
      setView('photos');
    };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(imported)) {
          // Merge with existing history, removing duplicates by date
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {view === 'compare' && compareSelection.length === 2 && (
        <div className="max-w-md mx-auto p-6">
          ...
          {(() => {
            const [older, newer] = compareSelection.sort((a, b) => new Date(a.date) - new Date(b.date));
